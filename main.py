...
def login(user, password):
    ...
    headers = {
        ...
        "User-Agent": "MiFit/5.3.0 (iPhone; iOS 14.4.1; Scale/3.00)"
    }
    data1 = {
        "client_id": "HuaMi",
        "password": f"{password}",
...
def main(user, passwd, step):
    ...
    if step == '':
        print("已设置为随机步数（18000-21000")
        step = str(random.randint(18000, 21000))
    login_token = 0
...
# 推送server
def push_wx(msg_url, desp=""):
    """
    推送消息到微信
    """
    if msg_url == '':
        print("[注意] 未提供推送地址，不进行推送！")
    else:
